...
union(){
 translate([-0.5,8.5,6]) flexbatterAA(n=4);

 linear_extrude(height = 8, center = false, convexity = 10)
   import (file = "chassis.dxf",layer="0");
   translate([0,0,0])linear_extrude(height = 25, center = false, convexity = 10)
   import (file = "chassis.dxf",layer="porte_crayon");
}

translate ([45,12,0])cube([2,4,30]);
translate ([45,70-15.5,0])cube([2,4,30]);
}
difference(){
